...
> 已删除 `PLATFORM_ADMIN`；历史数据请执行 `V5__remove_platform_admin.sql` 迁为 `INTERNAL_STAFF`。
...
### V4__ai_chat_and_caigouzhouqi.sql
AI 智能分析助手：

- `shangpinxinxi.caigouzhouqi`：可选采购周期（天）
- `ai_chat_session` / `ai_chat_message`：对话持久化表

也可直接执行仓库根目录旁的：`db/migration_ai_assistant.sql`
