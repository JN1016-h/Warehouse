		...
		<el-dropdown class="dropdown-box" @command="handleCommand" trigger="click">
			<div class="el-dropdown-link">
				<div class="avatar-identity">
					...
					<div class="identity-under-avatar">
						{{ identityText }}
					</div>
				</div>
		...
		computed: {
			...
			identityText() {
				const name = this.$store.getters['user/userName']
					...
					|| '用户'
				const role = this.$store.getters['user/roleDisplayName']
					|| (this.$storage.get('sessionTable') === 'users' ? '系统管理员' : (this.$storage.get('role') || '用户'))
				return name + ' - ' + role
			},
		},
		mounted() {
