...
export default {
	...
	mounted() {
		...
		if (!this.$storage.get('Token') || !sessionTable) {
			return
		}
		this.$http({
			...
			if (data && data.code === 0) {
				if(sessionTable == 'yonghu') {
					...
					// FR10：经销商不开放 AI 菜单
					if (data.data.userRole === 'DEALER' && this.menuList && this.menuList.backMenu) {
						this.menuList.backMenu = this.menuList.backMenu.filter(m => {
							if (!m.child || !m.child.length) return true
							return m.child[0].tableName !== 'aiAssistant'
						})
					}
				}
