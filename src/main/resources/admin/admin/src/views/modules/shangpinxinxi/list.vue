<template>
	<div class="main-content" :style='{"padding":"20px 30px","fontSize":"15px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 10px 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","background":"#fff","display":"flex"}' >
					...
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">颜色</label>
						<el-input v-model="searchForm.yanse" placeholder="颜色" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">尺寸</label>
						<el-input v-model="searchForm.chicun" placeholder="尺寸" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0","flexWrap":"wrap","background":"#fff","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('shangpinxinxi','商品库存')" type="success" @click="chartDialog1()">
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"14px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="gongyingshangid"
						label="供应商id">
						<template slot-scope="scope">
							{{scope.row.gongyingshangid}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
					...
					<el-table-column  :resizable='true' prop="fuzhuangtupian" width="200" label="服装图片">
						<template slot-scope="scope">
							<div v-if="scope.row.fuzhuangtupian">
								<img v-if="scope.row.fuzhuangtupian.substring(0,4)=='http'" :src="scope.row.fuzhuangtupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.fuzhuangtupian.split(',')[0])">
								<img v-else :src="$base.url+scope.row.fuzhuangtupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.fuzhuangtupian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('shangpinxinxi','出库')" @click="chukuxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								出库
							</el-button>
							<el-button class="btn8" v-if="isAuth('shangpinxinxi','入库')" @click="rukuxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								入库
							</el-button>
							<el-button class="btn8" v-if="isAuth('shangpinxinxi','订货')" @click="dinghuoxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								订货
							</el-button>
		...
		<dinghuoxinxi-cross-add-or-update v-if="dinghuoxinxiCrossAddOrUpdateFlag" :parent="this" ref="dinghuoxinxiCrossaddOrUpdate"></dinghuoxinxi-cross-add-or-update>
		...
		<el-dialog
			...
			width="800">
			<div id="fuzhuangkucunChart1" style="width:100%;height:600px;"></div>
...
<script>
	...
	import dinghuoxinxiCrossAddOrUpdate from "../dinghuoxinxi/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				dinghuoxinxiCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			chukuxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
			...
			dinghuoxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.dinghuoxinxiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','shangpinxinxi');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.dinghuoxinxiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.dinghuoxinxiCrossaddOrUpdate.init(row.id,type);
				});
			},
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var fuzhuangkucunChart1 = echarts.init(document.getElementById("fuzhuangkucunChart1"),'macarons');
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							fuzhuangkucunChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								fuzhuangkucunChart1.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			getDataList() {
				...
				let params = {
 				...
 				if(this.searchForm.fuzhuangbianhao!='' && this.searchForm.fuzhuangbianhao!=undefined){
				...
				if(this.searchForm.gongyingshangid!='' && this.searchForm.gongyingshangid!=undefined){
					params['gongyingshangid'] = this.searchForm.gongyingshangid
				}
 				...
 				if(this.searchForm.chicun!='' && this.searchForm.chicun!=undefined){
					params['chicun'] = '%' + this.searchForm.chicun + '%'
				}
