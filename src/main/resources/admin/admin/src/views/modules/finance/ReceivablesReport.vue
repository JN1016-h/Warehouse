	...
	<div class="main-content" :style='{"padding":"20px 30px","fontSize":"15px"}'>
		...
		<el-form class="center-form-pv" :style='{"margin":"0 10px 20px"}' :inline="true" :model="queryForm">
			<el-row :style='{"padding":"10px","background":"#fff","display":"flex"}'>
				...
				<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
					<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">客户名称</label>
					<el-input v-model="queryForm.customerName" placeholder="客户名称" @keydown.enter.native="handleQuery()" clearable></el-input>
				</div>
		...
		<el-card class="summary-card" :style='{"margin":"0 10px 20px","background":"#fff"}'>
			<div :style='{"display":"flex","justifyContent":"space-around","padding":"10px"}'>
				...
				<div class="summary-item" :style='{"textAlign":"center"}'>
					<div :style='{"color":"#999","fontSize":"14px","marginBottom":"10px"}'>已收款</div>
					<div class="amount paid" :style='{"color":"#67C23A","fontSize":"24px","fontWeight":"bold"}'>{{ formatAmount(summary.paidAmount) }}</div>
				</div>
				<div class="summary-item" :style='{"textAlign":"center"}'>
					<div :style='{"color":"#999","fontSize":"14px","marginBottom":"10px"}'>未收款</div>
		...
		<div :style='{"width":"100%","padding":"0 10px","fontSize":"14px","color":"#000"}'>
			<el-table class="tables"
				...
				<el-table-column :resizable='true' :sortable='true' prop="customerName" label="客户名称">
					<template slot-scope="scope">
						{{scope.row.customerName}}
					</template>
				</el-table-column>
				<el-table-column :resizable='true' :sortable='true' prop="amount" label="应收金额">
					<template slot-scope="scope">
...
export default {
	...
	methods: {
		...
		exportExcel() {
			...
			const url = `${this.$base.url}finance/receivables/export/excel?${queryString}`;
		...
		exportPDF() {
			...
			const url = `${this.$base.url}finance/receivables/export/pdf?${queryString}`;
			window.open(url, '_blank');
		},
